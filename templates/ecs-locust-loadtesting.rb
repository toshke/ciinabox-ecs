...
CloudFormation do
  ...
  config['availability_zones'].each do |az|
    Resource("SubnetPrivate#{az}") {
      ...
      Property('CidrBlock', FnJoin("", [FnFindInMap('EnvironmentType', 'ciinabox', 'NetworkPrefix'), ".", FnFindInMap('EnvironmentType', 'ciinabox', 'StackOctet'), ".", config['loadTestingSubnets']["SubnetOctet#{az}"], ".0/", FnFindInMap('EnvironmentType', 'ciinabox', 'SubnetMask')]))
  ...
  LaunchConfiguration('LaunchConfig') do
    ...
    InstanceType FnFindInMap('EnvironmentType', Ref('EnvironmentType'), 'LocustMasterInstanceType')
  ...
  LaunchConfiguration('LaunchConfigSlave') do
    ...
    InstanceType FnFindInMap('EnvironmentType', Ref('EnvironmentType'), 'LocustSlaveInstanceType')
    BlockDeviceMappings [{ DeviceName: '/dev/xvda', Ebs: { VolumeSize: 100, VolumeType: 'gp2' } }]
    UserData FnBase64(FnJoin('', [
        "#!/bin/bash\n",
        "INSTANCE_ID=$(echo `/opt/aws/bin/ec2-metadata -i | cut -f2 -d:`)\n",
  ...
  AutoScalingGroup('AutoScaleGroupMaster') do
  ...
  AutoScalingGroup('AutoScaleGroupSlave') do
    ...
    DesiredCapacity 0
    MaxSize 0
    VPCZoneIdentifier config['availability_zones'].collect { |az| Ref("SubnetPrivate#{az}") }
    TerminationPolicies ['NewestInstance']
  ...
  loadbalancers.each do |lb|
    ...
    Resource("#{lb["name"]}LoadBalancer") {
      ...
      if lb.key?('internal')
        Property("Subnets", config['availability_zones'].collect { |az| Ref("SubnetPrivate#{az}") })
    ...
    lb['records'].each do |record|
      Resource("#{record.gsub('*', 'Wildcard')}LoadBalancerRecord") {
        ...
        if record == "apex"
          ...
          Property("Name", FnJoin("", ["#{record}.", dns_domain, '.']))
        end
        Property("Type", "A")
